...
/** This class generates the best possible net for select group question behaviour
 * 
...
public class SelectGQBehaviourNet {
	...
	private final double INPUT[][] = { refineInput(1, 15, true, 2), refineInput(10, 15, true, 2), refineInput(20, 15, true, 2),
									   refineInput(1, 15, true, 6), refineInput(10, 15, true, 6), refineInput(20, 15, true, 6) };
	private final double IDEAL[][] = { { 0.5, 0.5 }, { 0.5, 0.5 }, { 0.5, 0.5 }, { 0.5, 0.5 }, { 0.5, 0.5 }, { 0.5, 0.5 } };
	
	public boolean ready_to_parse = false;
	
	public SelectGQBehaviourNet(boolean isConnected) {
	...
	public static void main(String[] args) {
		SelectGQBehaviourNet sgqb = new SelectGQBehaviourNet(false);
		sgqb.testoutput();
	}
	...
	public double[] getRate(int controlLevel, int age, boolean isMale, int prevGroup) {
		...
		BasicNetwork net = (BasicNetwork) EncogDirectoryPersistence.loadObject(new File(Util.SELECT_BEHAVIOUR_NET_EG));
	...
	public void writeFile(BasicNetwork network) {
		EncogDirectoryPersistence.saveObject(new File(Util.SELECT_BEHAVIOUR_NET_EG), network);
		ready_to_parse = true;
	}
}
