...
public class TimeSlotInterventionNet {
	...
	private final double INPUT[][] = { refineInput(1, 15, true), refineInput(10, 15, false), refineInput(20, 15, true),
			   						   refineInput(1, 15, false), refineInput(10, 15, true), refineInput(20, 15, false) };
	...
	public TimeSlotInterventionNet(boolean isConnected) {
	...
	public static void main(String[] args) {
		TimeSlotInterventionNet tsin = new TimeSlotInterventionNet(false);
		tsin.testoutput();
	}
	...
	public double[] getRate(int controlLevel, int age, boolean isMale) {
		...
		BasicNetwork net = (BasicNetwork) EncogDirectoryPersistence.loadObject(new File(Util.SLOT_INTERVENTION_NET_EG));
	...
	public void writeFile(BasicNetwork network) {
		EncogDirectoryPersistence.saveObject(new File(Util.SLOT_INTERVENTION_NET_EG), network);
		ready_to_parse = true;
	}
}
